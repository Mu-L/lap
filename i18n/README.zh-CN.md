...
- **RAW + JPEG/HEIC 配对**：可选择将同一文件夹中同名的 RAW 文件及其 JPEG 或 HEIC 配套文件显示为一个项目。原始文件仍保持独立；重命名、移动、复制、粘贴和删除时会一起处理。
...
- 保存内置图片编辑时，生成的图片会写入所选目标位置。这与仅保存在 Lap 中的显示旋转不同。
- 当您**在 Lap 中**重命名、移动、复制或删除文件时，Lap 会同时更新本地目录；也会将 Apple Live Photo 组件、AAE 附属文件和已启用的 RAW + JPEG/HEIC 配对等受支持的关联资源一起处理。
...
- 标签
- 注释
- 收藏、评分和审核状态（包括 Picks 与 Rejects）
- Lap 的显示旋转及其他仅限资料库的状态
- 智能相册及其规则、分组、排序和顺序
...
Lap 可以重新扫描文件夹并发现许多文件系统变化。不过，当文件在 Lap 外被重命名、移动、替换或复制时，Lap 无法保证本地关联信息会随文件迁移，尤其是收藏集归属和其他仅限资料库的数据。

如果您的工作流依赖收藏集、标签、注释、收藏、评分、审核状态或 Lap 的显示旋转，请尽可能使用 Lap 移动和重命名文件。如果您经常在 Lap 外管理文件，请将这些功能视为 Lap 本地的整理信息，并将 Lap 数据库和配置与照片一同备份。您可以在 **设置 → 存储** 中查看或更改数据库位置，并创建数据库备份。
