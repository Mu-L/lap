...
Lap 可以重新扫描文件夹并发现许多文件系统变化。不过，当文件在 Lap 外被重命名、移动、替换或复制时，Lap 无法保证本地关联信息会随文件迁移，尤其是合集归属和其他仅限资料库的数据。

如果您的工作流依赖合集、标签、注释、收藏、评分、选片状态或 Lap 的显示旋转，请尽可能使用 Lap 移动和重命名文件。如果您经常在 Lap 外管理文件，请将这些功能视为 Lap 本地的整理信息，并将 Lap 数据库和配置与照片一同备份。您可以在 **设置 → 存储** 中查看或更改数据库位置，并创建数据库备份。
