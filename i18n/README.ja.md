...
コレクション、タグ、コメント、お気に入り、評価、選別状態、またはLapの表示上の回転に依存するワークフローでは、可能な限りLapでファイルを移動・名前変更してください。Lapの外で頻繁に作業する場合は、これらの機能をLap内だけの整理情報として扱い、写真のバックアップとあわせてLapのデータベースと設定もバックアップしてください。データベースの場所の確認・変更、およびデータベースのバックアップ作成は、**設定 → ストレージ**から行えます。
