...
컬렉션, 태그, 댓글, 즐겨찾기, 평점, 선별 상태 또는 Lap의 표시 방향에 의존하는 작업 흐름이라면 가능한 한 Lap에서 파일을 이동하고 이름을 변경하세요. Lap 외부에서 자주 작업한다면, 이러한 기능은 Lap 내부의 로컬 정리 정보로 여기고 사진 백업과 함께 Lap의 데이터베이스와 구성도 백업하세요. 데이터베이스 위치를 확인하거나 변경하고 데이터베이스 백업을 만들려면 **설정 → 저장소**를 사용하세요.
